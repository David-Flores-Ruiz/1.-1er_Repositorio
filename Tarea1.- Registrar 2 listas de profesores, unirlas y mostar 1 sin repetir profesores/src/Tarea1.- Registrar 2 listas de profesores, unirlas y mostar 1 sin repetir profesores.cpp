...
int main(void) {
	...
	for(i=0; i<n3; i++ ){
		for(j=i; j<n3; j++){
			if((i != j) && ((apuntador + i)->calificacion != -5) && ((apuntador + j)->calificacion != -5))// Evitar que compare contra si mismo y Revisar si no ha habido BORRADO Lógico
			{
				valor = comparaCadenas( (apuntador + i)->nombre , (apuntador + j)->nombre );
				printf("Valor Comparacion: %d ", valor);
				if(valor == 0) //Si en la comparacion son iguales
				{
					(apuntador + i)->calificacion = (	(apuntador + i)->calificacion + (apuntador + j)->calificacion) /2;	//Le asignamos el promedio al nombre repetido en la posición que primero comparo contra todas la demás borrando logicamente esas otras
					(apuntador + j)->calificacion = -5;	//Borrado lógico
				}
				printf("------------");
				printf("%d %d", i, j);
			}
			...
			if(	(apunta + j)->calificacion < (apunta + j + 1)->calificacion	)
				{
					...
					strcpy(	(auxSwap)->nombre, (apunta + j)->nombre );	//Pasar al AUX
					(auxSwap)->calificacion = (apunta + j)->calificacion;	//Pasar al AUX

					strcpy(	(apunta + j)->nombre, (apunta + j + 1)->nombre );// Inicia el SWAP
					(apunta + j)->calificacion = (apunta + j + 1)->calificacion;//Inicia el SWAP

					strcpy(	(apunta + j + 1)->nombre, (auxSwap)->nombre	);//FIN de SWAP
					(apunta + j + 1)->calificacion = (auxSwap)->calificacion;//FIN de SWAP
				}
		}
	}
}
